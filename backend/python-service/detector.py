...
# USB webcams stall if we infer every frame at 640.
# Use lighter settings for local indexes.
...
def open_camera(source):
    """
    USB-tuned open:
    MJPG + 30fps + small buffer + warmup.
    ...
    if is_usb_index:
        ...
        # DSHOW first on Windows for USB cameras.
        # Then MSMF, then ANY.
        attempts = [
            (i, cv2.CAP_DSHOW),
            (i, cv2.CAP_MSMF),
    ...
    for src, backend in attempts:
        ...
        try:
            # 30fps requested.
            # USB cameras that cannot do it will ignore it.
        ...
        try:
            # Disable autofocus hunting on USB cameras.
            cap.set(cv2.CAP_PROP_AUTOFOCUS, 0)
        except Exception:
            pass

        # Warmup:
        # discard stale buffered frames so the first served frame is fresh.
        ok, frame = False, None
...
def grab_latest(cap):
    ...
    BUFFERSIZE=1 already drops stale frames.
    The old grab()+grab()+read() pattern stalls USB cameras
    and causes glitches.
    """
...
class PersonTracker:
    ...
    def __init__(self, backend_url=None):
        ...
        #
        # Production:
        # BACKEND_URL will contain the public Render backend URL.
        ...
        # hist[cam][id] =
        # deque of (cx, cy, w, h, t), maxlen 60
        ...
        self.behavior_counts = defaultdict(
            lambda: {
                "falls": 0,
                "crowdEvents": 0,
                "loitering": 0,
    ...
    def _draw(
        ...
        for i, (
            ...
            if f.get("fall"):
                ...
                tid is not None
                and tid in self.crossed[self._cam]
            ):
                color = (0, 140, 255)
    ...
    def _classify(
        ...
        """
        Return flags dict for one track.
        Uses trajectory history.
        """
        ...
        if len(q) >= 4:
            ...
            if (
                ...
                self.still_since[cam_id][tid] = (
                    self.still_since[cam_id].get(
                        tid,
                        now
                    )
                )
            ...
            # Stillness:
            # low movement over last ~2 seconds
            recent = list(q)[-12:]
        ...
        while self.active.get(cam_id):
            ...
            if frame is None:
                ...
                if fail_streak >= 30:
                    ...
                    # USB glitch / unplugged:
                    # try to reopen instead of
                    # serving frozen frames.
            ...
            # Pacing:
            # if inference is slower than the camera,
            # skip extra frames instead of queueing.
            if n % detect_every == 0:
            ...
            # Tiny yield.
            # No fixed sleep that would cap FPS.
            time.sleep(0.002)
